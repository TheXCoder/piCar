...
miscLogger = logging.getLogger("carMisc")
    ...
    for count in range(numberIntervals):
        try:
            if deviceIP != "":
                ...
                miscLogger.info("device @ \"" + device_IPOrName + "\" has been found")
            #this block will never get ran if the the first if statement ran
                ...
                miscLogger.info("device with name \"" + device_IPOrName + (".local" * isLocal) + "\" has been found")
            return deviceName, deviceIP
        except socket.gaierror or socket.herror:
            miscLogger.warning("device \"" + device_IPOrName + "\" has not been found. Waiting " + str(intervalWaitTime) + " second(s)")
...
def getConnected(serverIP : str, clientsocket,  port : int = 5050, numberIntervals : int = 5,  intervalWaitTime : float = 1):
    for _ in range(numberIntervals):
        if _ == numberIntervals -1: 
            miscLogger.warning("Last try before giving up")
        else:
            miscLogger.warning("Server has refused to connect. Trying again")
        try:
            clientsocket.connect((serverIP,5050))
            miscLogger.info("Connected to server")
            return clientsocket
        except ConnectionRefusedError:
            time.sleep(intervalWaitTime)
            
    miscLogger.error("System has failed to connect with the server.")
    return None