...
class EventConfiguration:
    ...
    def addTrustedDevice(self, levelNumber : int, deviceName : str) -> bool | rsa.PublicKey | None:
        self._readConfig()
        if checkTypeHard(levelNumber, int) is False or levelNumber < 0:
            raise TypeError()
            return None
        ...
        if self._config["level " + str(levelNumber)]["trustedDevices"] is None:
            ...
            deviceSigniture : str = deviceName + getRandomString(64)
            publicKey : rsa.PublicKey; privateKey : rsa.PrivateKey
            publicKey, privateKey = rsa.newkeys(512)
            trustedDevice : dict = {"publicKey" : publicKey.save_pkcs1("PEM").decode(encoding),
                                    "privateKey" : privateKey.save_pkcs1("PEM").decode(encoding)}
            self._config["level " + str(levelNumber)]["trustedDevices"] = {deviceSigniture : trustedDevice}
            self._writeConfig()
            return publicKey
        if any(deviceName in device[:-64] for device in self._config["level " + str(levelNumber)]["trustedDevices"].keys()):
            return False
        encoding : str = self._config["level " + str(levelNumber)]["encoding"]
        deviceSigniture : str = deviceName + getRandomString(64)
        publicKey : rsa.PublicKey; privateKey : rsa.PrivateKey
        ...
        trustedDevice : dict = {"publicKey" : publicKey.save_pkcs1("PEM").decode(encoding),
                                "privateKey" : privateKey.save_pkcs1("PEM").decode(encoding)}
        self._config["level " + str(levelNumber)]["trustedDevices"].update({deviceSigniture : trustedDevice})
        self._writeConfig()
        return publicKey
