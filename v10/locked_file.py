...
#-------------check function----------------------------------------------
    def checkFileHash(self):
        ...
        *logging warnings may appear if two instances of the same file are opended at the same time*
        ...
        with open(self._filePath, "rb") as file:
            fileSame = int(config[self._filename]["fileSigniture"]) == int(hash(file.read()))
        if not fileSame:
