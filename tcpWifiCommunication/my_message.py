...
class MyMessage:
    ...
    def __init__(self, eventsDictionary : dict | None = None):
        ...
        print("my message instance has been created")
...
#--------command handling---------------------------------------------------
    ...
    def command(self, commandName : str, data : dict | None,  hasPriority : bool = False) -> dict | None:
        ...
        """
        self._logger.debug("adding the command \"" + commandName + "\" to the que.")
...
#------message que --------------------------------------
    def _addToQue(self, command : Command , hasPriority : bool = False) -> True:
        ...
        """
        if hasPriority:
            self._logger.debug("High priority command \"" + command.getName() + "\"")
...
#----------- encoding message functions-----------
    ...
    def _getMessageFormat(self) -> tuple:
        """
        gets the format of the next message being sent
        if no encoding message has been sent yet,
        the message size will be 0
        the encoding will be an empty str

        Returns
        -------
        current message size : int
            the size of the next message
        current message encoding : str
            the encoding of the next message

        """
        return self._currentMessageSize, self._currentMessageEncoding
        
    
    def getMessageFormat(self) -> tuple:
        """
        gets the format of the next message being sent
        if no encoding message has been sent yet
        the message size will be the encoding message size
        the encoding will be the encoding message encoding
        ...
        """
        size : int = self._currentMessageSize if self._currentMessageSize > 0 else self._ENCODINGMESSAGESIZE
        encoding : str = self._currentMessageEncoding if self._currentMessageEncoding != "" else self._ENCODINGMESSAGEENCODING
...
#--------recieveMessage--------------------------
    ...
    def _getCommandMessage(self, messageRecieved : dict) -> None:
        ...
        function = None 
        try:
            self._eventsDictionary[messageRecieved["command"]]
        ...
        match recvMessage["command"]:
            ...
            case _:
                try:
                    ...
                    self._acceptDecline(recvMessage, -1, {"error" : "No function associated with command"})
