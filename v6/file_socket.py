...
class FileSocket(SocketHandler):
    ...
    def __init__(self, deviceSocket : socket.socket, eventsDictionary : dict = None,
        ...
        self._masterPath : str = os.getcwd() #current directory where exacution takes place
        self._folderToSaveDir : str = "" #directory to save files in
        
        if folderToSaveFilesDIR is None:
            self._folderToSaveDir = self._masterPath
        else:
            self._folderToSaveDir = folderToSaveFilesDIR
    ...
    def changeSaveFileDirectory(self, folderToSaveFiles : str):
    ...
    def _fileGenerator(self, filename : str, isBinary : bool = False) -> tuple:
        ...
        with FileManger(filename, fileMode) as file:
            chunkData : str = ''
            chunkIndex = 0
            yield "finfo", {"filename" : filename, "isBinary" : isBinary}
            while True:
...
#--------------recv message----------------------
    ...
    def _getFileEnd(self, recvMessage : dict, encoding : str):
        ...
        if data["filename"] in self._filesToRecv.keys():
            ...
            with FileManger(data["filename"], fileMode) as file:
                chunkIndex : int = 0
    ...
    def recv(self) -> dict:
        ...
        try:
            ...
            raise RuntimeError("socket has been terminated within the instance")
        return {"command" : None, "ID" : None}
