...
class MyMessage:
    ...
    def __init__(self, eventsDictionary : dict = None):
        ...
        self.__eventsDictionary : {str:Callable[[object],[bool,object]]} = {}
        try: #this test each function to see if the can be called
            ...
            self.__eventsDictionary = {}
            raise error
        else: #adds the events to event dictionary if no error has occured
            self.__eventsDictionary = copy.deepcopy(eventsDictionary)
...
#----------queing-------------------------------------------
    def __getNewID(self, command : str, data) -> int:
        ...
        return newID
    def __addToQue(self, newMessage : str, encoding : str = "utf-8", hasPriority : bool = False):
        """
        ...
        """
        if hasPriority:
            if self.__messageQue[0][0][0:4] == "size" or self.__messageQue[0][0][0:4] == "lmf ":
                newQue : tuple = *self.__makeEncodingMessage(newMessage, encoding), *self.__messageQue
            else:
                newQue : tuple = self.__messageQue[0], *self.__makeEncodingMessage(newMessage, encoding), *(self.__messageQue[1:])
            self.__messageQue = list(newQue)
            return None
        
        newQue : tuple = *self.__messageQue, *self.__makeEncodingMessage(newMessage, encoding)
        self.__messageQue = list(newQue)
        return None
...
#---------------builtin messages and commands---------------------------------------
    ...
    def echo(self, messageToEcho : str, encoding : str = "utf-8", hasPriority : bool = False)-> None:
        ...
        self.__addToQue(message, encoding, hasPriority)
        return None
    ...
    def __acceptAndDecline(self, recvMessage : dict, data, hasAccept : bool):
        """
...
#-----------------get message data and states----------------------
    ...
    def getQueList(self) -> tuple :
        ...
        """
        queList : tuple  = tuple(copy.deepcopy(self.__messageQue))
        return queList
...
#-----------Error Checking Functions
    ...
    #         function = self.__eventsDictionary["rply"]
...
#---------receive messages------------------------------------------------------------
    ...
    def __getEchoReplyMessage(self, recvMessage : dict, encoding : str) -> dict:
        ...
        if "rply" in self.__eventsDictionary.keys():
            ...
            self.__eventsDictionary["rply"], recvMessage["data"])
            if hasAccept:
    ...
    def recv(self, messageRecieved:str, encoding = "utf-8") -> dict:
        ...
        if recvMessage["command"] == "echo":
            ...
            self.__acceptAndDecline(recvMessage, None, False)
        self.__currentMessageSize = 0
        self.__currentMessageEncoding = ""
        return returnValue
