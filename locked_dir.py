...
class LockedDir:
    ...
    def checkFolderMode(self) -> str:
        return self._checkFolderMode()
    
    def setAllowedFileExtention(self, fileExtensions : list, password: str,
                                textEncoding : str = "utf-8") -> bool:
        """
        Sets the allowed file extention for folder
        example ".exe", ".txt", ".png", ".mp3"

        Parameters
        ----------
        fileExtensions : list
            list of allowed file extenstions
        password : str
            password for directory
        textEncoding : str, optional
            self explanitory. The default is "utf-8".

        Raises
        ------
        TypeError
            If a extenstion in the list does not start with "."

        Returns
        -------
        bool
            True if allowed extenstion list has been changed
            False if password has not been set or password is incorrect
            False if TypeError has been raised
    ...
    def checkFile(self, filenameWithExtenstion:str)->bool:
        """
        check to see if file type is allowed

        Parameters
        ----------
        filenameWithExtenstion : str
            filename with extension
            example "test.exe", "text.txt", "picture.png", "music.mp3"

        Returns
        -------
        bool
            True if file is allowed
            False if file is not allowed
    ...
    def _writeToConfig(self, config : configparser.ConfigParser | None)->bool:
        """
        write current config to the masterConfigFile
